...
int main(int argc, char **argv)
{
    ...
    while (bag.has_next())
    {
        ...
        // Process per-frame point clouds from lidar_scan_undistorted topic
        // This topic publishes per-frame registered points (always enabled, not dependent on visualize)
        if (msg->topic_name == "lidar_scan_undistorted") {
            RCLCPP_INFO(rclcpp::get_logger("2fast2lammaFrame"), "Received message on topic: lidar_scan_undistorted");
            ...
            if (!cloud_msg || cloud_msg->data.empty()) {
                ...
                RCLCPP_DEBUG(rclcpp::get_logger("2fast2lammaFrame"), "Empty PointCloud2 message on lidar_scan_undistorted");
                continue;
            }
    
            size_t num_points = cloud_msg->width * cloud_msg->height;
    
            RCLCPP_INFO(rclcpp::get_logger("2fast2lammaFrame"), "Processing %zu points from lidar_scan_undistorted", num_points);
            ...
            RCLCPP_INFO(rclcpp::get_logger("2fast2lammaFrame"), "Added %zu points (total: %zu)", num_points, points_global.size());
        }
        
        if (msg->topic_name == "/end_of_scan_odom") {
            RCLCPP_INFO(rclcpp::get_logger("2fast2lammaOdometry"), "Received message on topic: /end_of_scan_odom");
            ...
            if (!odom_msg) {
                RCLCPP_ERROR(rclcpp::get_logger("2fast2lammaOdometry"), "Odometry message deserialization error!");
                return 1;
            }
            ...
            RCLCPP_INFO(rclcpp::get_logger("2fast2lammaOdometry"), "Added position to trajectory: x=%.3f, y=%.3f, z=%.3f", x, y, z);
        }
    }
