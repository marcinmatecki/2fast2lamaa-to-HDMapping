...
ros2 launch ffastllamaa mid360_odometry.launch.py
'\'' C-m
...
echo "[record] start"
ros2 bag record /lidar_scan_undistorted /end_of_scan_odom \
...
echo "[play] start"
ros2 bag play '"$DATASET_CONTAINER_PATH"'; tmux wait-for -S BAG_DONE;
echo "[play] done"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    source /opt/ros/jazzy/setup.bash
    source /ros2_ws/install/setup.bash
    ros2 run 2fast2lamaa-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-2fast2lamaa\"
  "
